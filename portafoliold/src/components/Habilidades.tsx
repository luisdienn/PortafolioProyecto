...
import { FaReact, FaJava, FaGitAlt, FaDocker, FaUsers, FaComments, FaLightbulb, FaHandshake } from 'react-icons/fa';
...
const softSkills = [
    { icon: <FaLightbulb className="text-yellow-400 w-8 h-8" />, label: 'Liderazgo' },
    { icon: <FaUsers className="text-blue-400 w-8 h-8" />, label: 'Trabajo en equipo' },
    { icon: <FaComments className="text-green-400 w-8 h-8" />, label: 'Comunicación' },
    { icon: <FaHandshake className="text-pink-400 w-8 h-8" />, label: 'Adaptación al cambio' },
];
...
export default function Habilidades() {
    ...
    const handleMouseUp = (e: React.MouseEvent) => {
        ...
        if (Math.abs(diff) > 50) { // Sensibilidad del drag
    ...
    return (
        <>
            <h1 className="font-syne text-4xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white pt-32 text-center">Habilidades</h1>
            <div
            ...
            >
                ...
                >
                    {pages[page].items.map((skill, idx) => (
                        <div key={idx} className="relative flex flex-col items-center justify-center w-20 h-20">
                ...
                <div className="flex gap-2">
                    {pages.map((_, idx) => (
                        <button
                            key={idx}
                            className={`w-6 h-1  ${page === idx ? 'bg-white' : 'bg-gray-500'} transition`}
